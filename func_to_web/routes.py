...
async def handle_form_submission(
    request: Request, func: Callable, params: dict[str, ParamInfo]
) -> JSONResponse:
    ...
    try:
        ...
        for name, info in params.items():
            if info.is_list:
                ...
                if (
                    len(processed_list) == 1
                    and isinstance(processed_list[0], str)
                    and processed_list[0].startswith("[")
                ):
...
def setup_download_route(app: FastAPI):
    ...
    async def download_file(file_id: str):
        ...
        response = FastAPIFileResponse(
            path=path, filename=safe_filename, media_type="application/octet-stream"
        )

        return response


def _register_function_routes(
    app: FastAPI, func: Callable, templates: Jinja2Templates, has_auth: bool
):
    ...
    def make_form_handler(title: str, prms: dict, desc: str | None, submit_name: str):
        async def form_view(request: Request):
            flds = build_form_fields(prms)
            submit_url = request.url_for(submit_name)
    ...
    form_name = f"form_{func.__name__}"
    submit_name = f"submit_{func.__name__}"
    app.get(route, name=form_name)(
        make_form_handler(func_name, params, description, submit_name)
    )
    app.post(submit_route, name=submit_name)(make_submit_handler(func, params))


def setup_single_function_routes(
    app: FastAPI,
    func: Callable,
    params: dict,
    ...
    async def form(request: Request):
        ...
        return templates.TemplateResponse(
            "form.html",
            {
                ...
                "fields": fields,
                "submit_url": request.url_for("submit_single"),
                "show_back_button": False,
                "has_auth": has_auth,
            },
        )

    @app.post("/submit", name="submit_single")
    async def submit(request: Request):
        return await handle_form_submission(request, func, params)


def setup_multiple_function_routes(
    app: FastAPI, funcs: list[Callable], templates: Jinja2Templates, has_auth: bool
):
        ...
        tools = [
            {
                ...
                "path": request.url_for(f"form_{f.__name__}"),
            }
            for f in funcs
        ]
        return templates.TemplateResponse(
            "index.html", {"request": request, "tools": tools, "has_auth": has_auth}
        )
...
def setup_grouped_function_routes(
    app: FastAPI,
    ...
    async def index(request: Request):
        ...
        return templates.TemplateResponse(
            "index.html", {"request": request, "groups": groups, "has_auth": has_auth}
        )
